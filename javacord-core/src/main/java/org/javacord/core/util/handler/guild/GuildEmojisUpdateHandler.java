        ...
        api.getAllServerById(id).map(server -> (ServerImpl) server).ifPresent(server -> {
            ...
            emojis.forEach((key, value) -> {
                ...
                if (optionalEmoji.isPresent()) {
                    ...
                    if (!Objects.deepEquals(oldName, newName)) {
                        ...
                        EventDispatchUtil.dispatchToKnownCustomEmojiChangeNameListeners(
                                server,
                                emoji,
                                server,
                                api,
                    ...
                    if (!newWhitelist.containsAll(oldWhitelist) || !oldWhitelist.containsAll(newWhitelist)) {
                        ...
                        EventDispatchUtil.dispatchToKnownCustomEmojiChangeWhitelistedRolesListeners(
                                server,
                                emoji,
                                server,
                                api,
                    ...
                    EventDispatchUtil.dispatchToKnownCustomEmojiCreateListeners(
                            server,
                            server,
                            api,
            ...
            server.getCustomEmojis().stream()
                    ...
                    .forEach(emoji -> {
                        ...
                        EventDispatchUtil.dispatchToKnownCustomEmojiDeleteListeners(
                                server,
                                emoji,
                                server,
                                api,
